...
<!-- diagram meta: type=architecture; nodes=左に Actor、右に Function。Function は F01-F07。F01 は準備、F02 は整理、F03 は対象選定、F04 は体験構造化、F05 は提案核言語化、F06 はプロトタイプ前判断、F07 は実装前引き渡し; edges=バンドメンバーは F01-F07 に接続、後から参加するバンドメンバーは F02-F05 に接続、バンドリーダー / バンドマネージャーは F03-F05 に接続 -->
...
[[diagram:f01-usecase]]
<!-- diagram meta: type=architecture; nodes=左に バンドメンバー、右に 役割分担を揃える、仮説を揃える、進行カードを作る; edges=バンドメンバー から 3 つの use case に接続 -->
...
[[diagram:f02-usecase]]
<!-- diagram meta: type=architecture; nodes=左に バンドメンバー と 後から参加するバンドメンバー、右に 論点を切り出す、次回確認事項を残す、共有可能な整理物にする; edges=バンドメンバー は 3 つ全てに接続、後から参加するバンドメンバー は 共有可能な整理物にする に接続 -->
...
[[diagram:f03-usecase]]
<!-- diagram meta: type=architecture; nodes=左に バンドメンバー と バンドリーダー / バンドマネージャー、右に 候補を比較する、Counter-persona を置く、Primary の判断材料を揃える; edges=両 Actor から 3 つの use case に接続 -->
...
[[diagram:f04-usecase]]
<!-- diagram meta: type=architecture; nodes=左に バンドメンバー、右に 現状の詰まりを可視化する、理想の流れを描く、提案の中心シーンを選ぶ; edges=バンドメンバー から 3 つの use case に接続 -->
...
[[diagram:f05-usecase]]
<!-- diagram meta: type=architecture; nodes=左に バンドメンバー と バンドリーダー / バンドマネージャー、右に Why/Who/What を揃える、提案の一文を作る、後続に渡す共通言語を残す; edges=バンドメンバー は 3 つ全てに接続、バンドリーダー / バンドマネージャー は Why/Who/What を揃える と 提案の一文を作る に接続 -->
...
[[diagram:f06-usecase]]
<!-- diagram meta: type=architecture; nodes=左に バンドメンバー、右に 見せる内容の優先順位を決める、禁止事項や避ける表現を揃える; edges=バンドメンバー から 2 つの use case に接続 -->
...
[[diagram:f07-usecase]]
<!-- diagram meta: type=architecture; nodes=左に バンドメンバー と 実装接続を担うメンバー、右に 概念をそのまま渡す、必要な案件だけ後段スキルを追加する; edges=バンドメンバー は 2 つ全てに接続、実装接続を担うメンバー は 概念をそのまま渡す に接続 -->
