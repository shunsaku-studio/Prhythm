...
allowed-tools: Bash(node:*), Bash(pnpm:*), Bash(git:*), Bash(which:*), Bash(ls:*), Bash(mkdir:*), Bash(python:*), Read, Glob, Grep, Agent, Write, Skill
---
...
- 図は Mermaid の `flowchart LR` で Markdown 内に直接書く
- **図はユースケース図だけ**にする。フロー図は出さない
- HTML / SVG 生成はこのスキルの成果物に含めない
...
- Mermaid 図は GitHub や対応 Markdown Preview で表示できます
```
