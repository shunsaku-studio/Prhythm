---
name: usecase-mapper
description: Persona・ビジョン・要件メモなどから、コード不要のユースケース図スケッチを生成する。全体俯瞰図と機能別ユースケース図を主役に `docs/usecase-map.md` へ出力し、一覧は補助情報として添える。
context: fork
...
このスキルの主役は **ユースケース図**。機能一覧やユースケース一覧は、図を描くための補助情報として扱う。
...
- `全体ユースケース図` を最上位に置く
- `Actor / Persona 一覧` と `機能一覧` は図を読むための補助として添える
- 各機能の details セクションは図優先でよい

### Mode B の補足

- 対象機能のユースケース図を最優先で出力する
- `全体ユースケース図` は省略可
- 一覧表は短くてよい
